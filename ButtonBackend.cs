...
public class VideoButton : MonoBehaviour
{
    ...
    private void OnClick()
    {
        ...
        else
        {
            ...
            if (length > 0)
            {
                lengthText.text = "Camera Length: " + length.ToString() + " units";
            }
            else
            {
    ...
    private float CalculateLength()
    {
        float length = 0f;
        RaycastHit hit;
        if (Physics.Raycast(startPos, camera.transform.forward, out hit))
        {
            float startDist = Vector3.Distance(startPos, hit.point);
            if (Physics.Raycast(endPos, camera.transform.forward, out hit))
            {
